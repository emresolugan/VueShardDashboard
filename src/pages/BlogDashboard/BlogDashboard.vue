  ...
  <div class="hello">
    ...
    <div class="card tableDiv">
      <div class="card-body cardBodyClass">
        ...
        <hr />
        <table
          v-if="firstTableShowing"
          class="table table-striped table-bordered clientTableClass"
          id="pdfScreen"
          ...
          <thead>
            <tr>
              ...
              <th scope="col" colspan="2" data-html2canvas-ignore="true" class="iconColClass">
                <i class="fa fa-plus-circle fa-2x addIconClass" v-on:click="routeFunc('/addblog')"></i>
              </th>
            </tr>
          </thead>
          <tbody>
        ...
        >
          <thead>
            <tr>
              ...
              <th scope="col">Surname</th>
              <th scope="col" colspan="2" data-html2canvas-ignore="true" class="iconColClass">
                <i class="fa fa-plus-circle fa-2x addIconClass"></i>
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              ...
              <td>Kahraman</td>
              <td class="iconColClass" data-html2canvas-ignore="true">
                <i class="fa fa-edit fa-lg editIconClass"></i>
              </td>
              <td class="iconColClass" data-html2canvas-ignore="true">
                <i class="fa fa-minus-circle fa-lg deleteIconClass"></i>
              </td>
            </tr>
            <tr>
              <td>23</td>
    ...
    <div class="card" v-if="firstTableShowing">
      <div class="card-body" style="padding-bottom: 0;">
        <div v-if="pageCount < 7" class="pagingArea">
          ...
          <p
            class="pagingNumbers"
            v-on:click="goSelectedPage(index)"
        ...
        <div class="form-group inputClass">
          <input
            ...
            v-on:keyup="goEnteredPage($event)"
            class="form-control"
            placeholder="Ex: 3"
          />
        </div>
  ...
  methods: {
      ...
      GetAll("http://localhost:5000/api/User/GetAll", headers).then(
        response => {
          if (typeof response == "number") {
            ...
            this.pageCount = Math.ceil(this.copyConsumerList.length / this.pageDataCount); // ceil yukarıya yuvarlar
          }
        }
      );
    },
    ...
    goNextPage: function() {
      if (this.pageNum < this.pageCount) {
        this.consumerList = [];
        this.consumerList = this.copyConsumerList.slice(
          this.pageNum * this.pageDataCount,
          (this.pageNum + 1) * this.pageDataCount
        );
        this.pageNum++;
      } else {
    ...
    goSelectedPage(val) {
      this.consumerList = [];
      this.consumerList = this.copyConsumerList.slice(
        (val - 1) * this.pageDataCount,
        val * this.pageDataCount
      );
      this.pageNum = val;
    },
    ...
    goEnteredPage(event) {
      ...
      if (
        ...
        Number(event.target.value) > this.pageCount
      ) {
        alert("Index is out of range!!");
      } else {
        this.consumerList = [];
        this.consumerList = this.copyConsumerList.slice(
          (Number(event.target.value) - 1) * this.pageDataCount,
          Number(event.target.value) * this.pageDataCount
        );
